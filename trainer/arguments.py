...
def parse_grpo_args():
    parser = argparse.ArgumentParser(description="TinyU-LLM GRPO 训练全局参数配置")

    # ================= 1. 基础训练与 GRPO 超参数 =================
    parser.add_argument("--epochs", type=int, default=1, help="外层经验收集(Rollout)的总轮数")
    parser.add_argument("--grpo_epochs", type=int, default=1, help="GRPO内层使用同一批数据更新的轮数")
    parser.add_argument("--rollout_batch_size", type=int, default=1, help="单卡用于生成经验(Rollout)的 Prompt Batch Size")
    parser.add_argument("--accumulation_steps", type=int, default=1, help="GRPO阶段的梯度累积步数")
    
    # 学习率与优化 (去除了 Critic 的学习率)
    parser.add_argument("--actor_learning_rate", type=float, default=1e-5, help="Actor(策略)模型最大学习率")
    parser.add_argument("--max_grad_norm", type=float, default=1.0, help="梯度裁剪阈值")
    parser.add_argument("--dtype", type=str, default="bfloat16", choices=["float16", "bfloat16", "float32"], help="训练时使用的精度")
    parser.add_argument("--seed", type=int, default=42, help="全局随机种子")
    ...
    # GRPO 专属核心超参数
    parser.add_argument("--group_size", type=int, default=4, help="每个 Prompt 生成的回答数量 G (通常设为 4 到 8)")
    parser.add_argument("--kl_coef", type=float, default=0.1, help="KL 散度惩罚系数，防止策略退化")
    parser.add_argument("--cliprange", type=float, default=0.2, help="Actor 策略更新的截断比率 (Policy clip)")
    ...
    # ================= 3. 路径与保存配置 =================
    # GRPO 专属路径组 (去除了 Critic 模型路径的说明，保留 SFT 和 RM)
    parser.add_argument("--data_path", type=str, default="../dataset/test_data.jsonl", help="GRPO Prompt 训练数据集路径")
    parser.add_argument("--actor_model_path", type=str, default="../out/sft_model.pth", help="SFT模型权重路径(用于初始化Actor和Reference)")
    parser.add_argument("--reward_model_path", type=str, default="../out/reward_model.pth", help="奖励模型权重路径(用于初始化Reward Model)")
    
    # 常规保存路径
    parser.add_argument("--checkpoint_dir", type=str, default="../checkpoints", help="GRPO Checkpoint 保存目录")
    parser.add_argument("--output_dir", type=str, default="../out", help="GRPO 最终权重保存目录")
    parser.add_argument("--save_steps", type=int, default=1, help="每隔多少次更新保存一次 Checkpoint")

    # ================= 4. 日志与实验追踪 =================
    parser.add_argument("--log_interval", type=int, default=1, help="每隔多少步打印一次日志(GRPO更新步数)")
    parser.add_argument("--project_name", type=str, default="TinyU-LLM-GRPO", help="SwanLab 项目名称")
    parser.add_argument("--run_name", type=str, default="grpo-run-1", help="本次实验名称")

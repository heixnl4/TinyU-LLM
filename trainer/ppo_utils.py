...
def init_ppo_models(config, model_paths, device):
    ...
    # PPO 中 Critic 和 Reward 的缺失参数（如 value_head）属于正常现象，不报警告
    ignore_missing = ["Critic", "Reward"]
    ...
    load_weights_safely(critic, rm_path or sft_path, device, "Critic", ignore_missing)
